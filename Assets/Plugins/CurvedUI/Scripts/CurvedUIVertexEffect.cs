...
public partial class CurvedUIVertexEffect : BaseVertexEffect {
    ...
    public override void ModifyMesh (Mesh mesh) {
        ...
        void CheckTextFontMaterial()
        {
            ...
            if (myText)
            {
                Vector2 myTextCachedGeneratorVerts = myText.cachedTextGenerator.verts[0].uv0;
                
                if (myText.cachedTextGenerator.verts.Count > 0 && myTextCachedGeneratorVerts != savedTextUV0)
                {
        ...
        UIVertex TesselateQuad(UIVertex[] quad, float x, float y)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Vector2 weights_i = quad[i].uv0 * weights[i];
                Vector2 quad_i = quad[i].uv1 * weights[i];
                uv0 += weights_i;
                uv1 += quad_i;
