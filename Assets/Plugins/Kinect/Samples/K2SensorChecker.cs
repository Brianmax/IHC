...
//			}
			...
			if(sensorInterface.InitSensorInterface(true, ref bNeedRestart))
			{
				...
				else
				{
					...
					if(infoText != null)
					{
						infoText.GetComponent<UnityEngine.UI.Text>().text = bSensorAvailable ? "Sensor is connected." : "No sensor is connected.";
					}
				}
			}
			else
		...
		catch (Exception ex) 
		{
			...
			if(infoText != null)
			{
				infoText.GetComponent<UnityEngine.UI.Text>().text = ex.Message;
			}
		}
