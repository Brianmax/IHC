...
//	}
	...
	public bool StartRecording()
	{
		...
		if(filePath.Length == 0)
		{
			...
			if(infoText != null)
			{
				infoText.GetComponent<UnityEngine.UI.Text>().text = "No file to save.";
			}
		}
		...
		if(isRecording)
		{
			...
			if(infoText != null)
			{
				infoText.GetComponent<UnityEngine.UI.Text>().text = "Recording... Say 'Stop' to stop the recorder.";
			}
	...
	public bool StartPlaying()
	{
		...
		if(filePath.Length == 0 || !File.Exists(filePath))
		{
			...
			if(infoText != null)
			{
				infoText.GetComponent<UnityEngine.UI.Text>().text = "No file to play.";
			}
		}
		...
		if(isPlaying)
		{
			...
			if(infoText != null)
			{
				infoText.GetComponent<UnityEngine.UI.Text>().text = "Playing... Say 'Stop' to stop the player.";
			}
	...
	public void StopRecordingOrPlaying()
	{
		if(isRecording)
		{
			...
			if(infoText != null)
			{
				infoText.GetComponent<UnityEngine.UI.Text>().text = "Recording stopped.";
			}
		}
		...
		if(isPlaying)
		{
			...
			if(infoText != null)
			{
				infoText.GetComponent<UnityEngine.UI.Text>().text = "Playing stopped.";
			}
		}

		if(infoText != null)
		{
			infoText.GetComponent<UnityEngine.UI.Text>().text = "Say: 'Record' to start the recorder, or 'Play' to start the player.";
		}
	}
	...
	void Start()
	{
		...
		else
		{
			...
			if(infoText != null)
			{
				infoText.GetComponent<UnityEngine.UI.Text>().text = "KinectManager not found, probably not initialized.";
			}
		}
			...
			if(manager && manager.IsInitialized())
			{
				...
				if(sBodyFrame.Length > 0)
				{
					using(StreamWriter writer = File.AppendText(filePath))
					{
						...
						if(infoText != null)
						{
							infoText.GetComponent<UnityEngine.UI.Text>().text = string.Format("Recording @ {0}s., frame {1}. Say 'Stop' to stop the player.", sRelTime, fCurrentFrame);
						}
	...
	private bool ReadLineFromFile()
	{
		...
		if(sLineParts.Length >= 2)
		{
			...
			if(infoText != null)
			{
				infoText.GetComponent<UnityEngine.UI.Text>().text = string.Format("Playing @ {0:F3}s., frame {1}. Say 'Stop' to stop the player.", fPlayTime, fCurrentFrame);
			}
