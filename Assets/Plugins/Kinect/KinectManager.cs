...
//		}
	...
	void Awake()
	{
		...
		catch (Exception ex) 
		{
			...
			{
				calibrationText.GetComponent<UnityEngine.UI.Text>().text = ex.Message;
			}
		}
...
//				dwFlags |= KinectInterop.FrameSource.TypeAudio;
		...
		catch(DllNotFoundException ex)
		{
			...
			if(calibrationText != null)
			{
				calibrationText.GetComponent<UnityEngine.UI.Text>().text = message;
			}
...
//		kinectReaderRunning = true;
		...
		if(calibrationText != null)
		{
			calibrationText.GetComponent<UnityEngine.UI.Text>().text = "WAITING FOR USERS";
		}
...
//						}
	...
	private void ProcessBodyFrameData()
	{
		   ...
		   calibrationText != null)
		{
			...
			calibrationText.GetComponent<UnityEngine.UI.Text>().text = string.Format("Sensor Height: {0:F1} m, Angle: {1:F0} deg", sensorData.sensorHgtDetected, -angle);
		}
...
//				{
//					calibrationText.GetComponent<UnityEngine.UI.Text>().text = string.Format("{0} - BodyAngle: {1:000}", 
...
//				}
				...
				if(liPrimaryUserId == 0 && aUserIndexIds.Length > 0)
				{
					...
					if(liPrimaryUserId != 0)
					{
						if(calibrationText != null && calibrationText.GetComponent<UnityEngine.UI.Text>().text != "")
						{
							calibrationText.GetComponent<UnityEngine.UI.Text>().text = "";
						}
					}
				}
	...
	private void CheckForGestures(Int64 UserId)
	{
		...
		if(Time.realtimeSinceStartup >= gesturesTrackingAtTime[UserId])
		{
			...
			if(gesturesDebugText && (UserId == liPrimaryUserId))
			{
				...
				gesturesDebugText.GetComponent<UnityEngine.UI.Text>().text = sDebugGestures;
			}
		}
	}
