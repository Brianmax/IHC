...
public class FacetrackingManager : MonoBehaviour 
{
	...
	void Start() 
	{
		...
		catch(DllNotFoundException ex)
		{
			...
			if(debugText != null)
				debugText.GetComponent<UnityEngine.UI.Text>().text = "Please check the Kinect and FT-Library installations.";
		}
		catch (Exception ex) 
		{
			Debug.LogError(ex.ToString());
	...
	void OnGUI()
	{
		if(isFacetrackingInitialized)
		{
			if(debugText != null)
			{
				if(isTrackingFace)
				{
					debugText.GetComponent<UnityEngine.UI.Text>().text = "Tracking - BodyID: " + primaryUserID;
				}
				else
				{
					debugText.GetComponent<UnityEngine.UI.Text>().text = "Not tracking...";
				}
			}
		}
	}
