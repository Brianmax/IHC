...
//		LeftHandCursor,
	...
	public virtual int[] GetNeededJointIndexes(KinectManager manager)
	{
        ...
        int[] neededJointIndexes = {
			...
			leftAnkleIndex, rightAnkleIndex, headIndex
        };
	...
	public virtual void CheckForGesture(long userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
        ...
        float gestureRight = (jointsPos[rightHipIndex].x + jointsPos[rightShoulderIndex].x) / 2.0f;

        //float gestureLeft = jointsPos[leftHipIndex].x;
        float gestureLeft = (jointsPos[leftHipIndex].x + jointsPos[leftShoulderIndex].x) / 2.0f;
        ...
        switch (gestureData.gesture)
        {
            ...
            case Gestures.HandsUp:
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                        if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[headIndex] &&
                            ((jointsPos[leftHandIndex].y - jointsPos[headIndex].y) > 0.1f) &&
                    ...
                    case 1:  // gesture phase 2 = complete
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            bool isInPose = (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[headIndex] &&
                            ((jointsPos[leftHandIndex].y - jointsPos[headIndex].y) > 0.1f) &&
                            ((jointsPos[rightHandIndex].y - jointsPos[headIndex].y) > 0.1f));
                ...
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                        if( 
                            jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] &&
                            jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] &&
                            jointsTracked[shoulderCenterIndex] &&
                            jointsPos[leftHandIndex].x >= gestureLeft && jointsPos[rightHandIndex].x <= gestureRight //&&
                            /*jointsPos[leftElbowIndex].z <= jointsPos[shoulderCenterIndex].z && jointsPos[rightElbowIndex].z <= jointsPos[shoulderCenterIndex].z*/)
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                            gestureData.progress = 0.1f;
                        }
