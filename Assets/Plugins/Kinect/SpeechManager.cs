...
public class SpeechManager : MonoBehaviour 
{
	...
	void Start() 
	{
		try 
		{
			...
			if(debugText != null)
			{
				debugText.GetComponent<UnityEngine.UI.Text>().text = "Please, wait...";
			}
...
//				AddGrammarPhrase("addressBook", string.Empty, "April Reagan", false, true);
			...
			if(debugText != null)
			{
				debugText.GetComponent<UnityEngine.UI.Text>().text = "Ready.";
			}
		} 
		catch(DllNotFoundException ex)
		{
			...
			if(debugText != null)
				debugText.GetComponent<UnityEngine.UI.Text>().text = "Please check the Kinect and SAPI installations.";
		}
		catch (Exception ex) 
		{
			Debug.LogError(ex.ToString());
			if(debugText != null)
				debugText.GetComponent<UnityEngine.UI.Text>().text = ex.Message;
		}
	}
	...
	void OnGUI()
	{
		if(sapiInitialized)
		{
			if(debugText != null)
			{
				...
				{
					debugText.GetComponent<UnityEngine.UI.Text>().text = string.Format("{0}  ({1:F1}%)", phraseTagRecognized, phraseConfidence * 100f);
				}
				else if(isListening)
				{
					debugText.GetComponent<UnityEngine.UI.Text>().text = "Listening...";
				}
			}
		}
	}
