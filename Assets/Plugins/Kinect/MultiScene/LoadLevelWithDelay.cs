...
public class LoadLevelWithDelay : MonoBehaviour 
{
	...
	void Start()
	{
		...
		if(validateKinectManager && debugText != null)
		{
			...
			{
				debugText.GetComponent<UnityEngine.UI.Text>().text = "KinectManager is not initialized!";
				levelLoaded = true;
			}
		}
	}
	...
	void Update() 
	{
		if(!levelLoaded && nextLevel >= 0)
		{
			...
			else
			{
				...
				if(debugText != null)
				{
					debugText.GetComponent<UnityEngine.UI.Text>().text = string.Format("Time to the next level: {0:F0} s.", timeRest);
				}
			}
		}
	}
